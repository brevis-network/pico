use super::{
    columns::{Poseidon2Cols, Poseidon2PreprocessedCols},
    Poseidon2SkinnyChip,
};
use crate::{
...
impl<const DEGREE: usize, Config: Poseidon2Config, F: Field> BaseAir<F>
...
impl<const DEGREE: usize, Config: Poseidon2Config, F: Field, CB: ChipBuilder<F>> Air<CB>
    for Poseidon2SkinnyChip<DEGREE, Config, F>
where
    CB::Var: Sized,
{
    fn eval(&self, builder: &mut CB) {
        ...
        let local_row: &Poseidon2Cols<_, Config> = (*local_row).borrow();
        let next_row: &Poseidon2Cols<_, Config> = (*next_row).borrow();
...
impl<const DEGREE: usize, Config: Poseidon2Config, F: Field>
    Poseidon2SkinnyChip<DEGREE, Config, F>
{
    fn eval_input_round<CB: ChipBuilder<F>>(
        &self,
        builder: &mut CB,
        local_row: &Poseidon2Cols<CB::Var, Config>,
        prep_local: &Poseidon2PreprocessedCols<CB::Var>,
        next_row: &Poseidon2Cols<CB::Var, Config>,
    ) {
