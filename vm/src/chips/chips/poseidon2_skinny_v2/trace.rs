use super::{
    columns::{
        Poseidon2Cols, Poseidon2PreprocessedCols, NUM_POSEIDON2_COLS,
        NUM_POSEIDON2_PREPROCESSED_COLS,
    },
    utils::{
        external_linear_layer, internal_linear_layer, NUM_EXTERNAL_ROUNDS, NUM_INTERNAL_ROUNDS,
        WIDTH,
    },
    Poseidon2SkinnyChip,
};
...
impl<const DEGREE: usize, F: PrimeField32> ChipBehavior<F> for Poseidon2SkinnyChip<DEGREE, F> {
    ...
    fn generate_preprocessed(&self, program: &Self::Program) -> Option<RowMajorMatrix<F>> {
        ...
        pad_to_power_of_two::<NUM_POSEIDON2_PREPROCESSED_COLS, F>(&mut trace.values);
...
impl<const DEGREE: usize, F: PrimeField32> Poseidon2SkinnyChip<DEGREE, F> {
    fn populate_external_round(&self, round_state: &[F; WIDTH], r: usize) -> [F; WIDTH] {
        let mut state = {
            // Add round constants.
