use super::{
    columns::{
        ...
        KOALABEAR_NUM_POSEIDON2_COLS, NUM_POSEIDON2_PREPROCESSED_COLS,
    },
    Poseidon2SkinnyChip,
};
use crate::{
...
macro_rules! implement_poseidon2_skinny_chip {
    ($num_external_rounds:expr, $num_internal_rounds:expr, $num_poseidon2_col:expr, $internal_round_idx:expr, $input_round_idx:expr, $output_round_idx:expr) => {
        impl<const DEGREE: usize, F: PrimeField32> ChipBehavior<F>
            ...
            Poseidon2SkinnyChip<DEGREE, $num_external_rounds, $num_internal_rounds, F>: BaseAir<F>,
        {
            ...
            fn generate_preprocessed(&self, program: &Self::Program) -> Option<RowMajorMatrix<F>> {
                            ...
                            (0..PERMUTATION_WIDTH).for_each(|j| {
                                cols.round_counters_preprocessed.round_constants[j] =
                                    if is_external_round {
                                        ...
                                        F::from_wrapped_u32(
                                            RC_16_30_U32[$num_external_rounds / 2 + j][0],
                                        )
                                    } else {
                                        F::ZERO
                                    };
                ...
                pad_to_power_of_two::<NUM_POSEIDON2_PREPROCESSED_COLS, F>(
                    &mut trace.values,
                    log_size,
                );
            ...
            fn generate_main(
                ...
                for event in &input.poseidon2_events {
                    ...
                    // modified in the computation of the first row.
                    {
                        ...
                        let input_cols: &mut Poseidon2Cols<F, { $num_internal_rounds - 1 }> =
                            first_row[0].as_mut_slice().borrow_mut();
                        input_cols.state_var = event.input;

                        let next_cols: &mut Poseidon2Cols<F, { $num_internal_rounds - 1 }> =
                    ...
                    for i in 1..$output_round_idx {
                        let next_state_var = {
                            let cols: &mut Poseidon2Cols<F, { $num_internal_rounds - 1 }> =
                        ...
                        let next_row_cols: &mut Poseidon2Cols<F, { $num_internal_rounds - 1 }> =
                    ...
                    {
                        let last_row_cols: &Poseidon2Cols<F, { $num_internal_rounds - 1 }> =
                ...
                pad_to_power_of_two::<$num_poseidon2_col, F>(&mut trace.values, log_size);
        ...
        impl<const DEGREE: usize, F: PrimeField32>
