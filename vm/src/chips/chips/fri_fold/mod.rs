...
#[derive(Default)]
pub struct FriFoldChip<const DEGREE: usize, F> {
...
impl<const DEGREE: usize, F: Field> BaseAir<F> for FriFoldChip<DEGREE, F> {
    fn width(&self) -> usize {
        NUM_FRI_FOLD_COLS
    }
}

impl<F: PrimeField32, const DEGREE: usize> ChipBehavior<F> for FriFoldChip<DEGREE, F> {
...
impl<const DEGREE: usize, F: Field> FriFoldChip<DEGREE, F> {
    pub fn eval_fri_fold<AB: ChipBuilder<F>>(
        &self,
        builder: &mut AB,
...
impl<AB, const DEGREE: usize, F: Field> Air<AB> for FriFoldChip<DEGREE, F>
where
    AB: ChipBuilder<F>,
{
