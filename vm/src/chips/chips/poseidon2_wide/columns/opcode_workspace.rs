...
pub struct AbsorbWorkspace<T: Copy> {
    ...
    pub num_remaining_rows_is_zero: IsZeroGadget<T>,
    ...
    pub last_row_ending_cursor_is_seven: IsZeroGadget<T>, /* Needed when doing the
...
impl<T: Copy> AbsorbWorkspace<T> {
    pub(crate) fn is_last_row<F: Field, AB: ChipBuilder<F>>(&self) -> AB::Expr
    where
        T: Into<AB::Expr>,
    {
    ...
    pub(crate) fn do_perm<F: Field, AB: ChipBuilder<F>>(&self) -> AB::Expr
    where
        T: Into<AB::Expr>,
    {
    ...
    pub(crate) fn num_consumed<F: Field, AB: ChipBuilder<F>>(&self) -> AB::Expr
    where
        T: Into<AB::Expr>,
    {
        self.is_syscall_not_last_row.into()
...
pub struct FinalizeWorkspace<T: Copy> {
    ...
    pub state: [T; WIDTH],
    pub state_cursor: T,
    pub state_cursor_is_zero: IsZeroGadget<T>,
}
