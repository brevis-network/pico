use super::super::{
    columns::CpuCols, memory::columns::MemoryColumns, opcode_selector::columns::OpcodeSelectorCols,
    CpuChip,
};
use crate::{
    chips::{
...
impl<F: Field> CpuChip<F> {
    ...
    pub(crate) fn eval_memory_address_and_access<CB: ChipBuilder<F>>(
        ...
        builder
            ...
            .assert_eq::<CB::Expr, CB::Expr>(
                ...
                memory_columns.addr_word.reduce::<CB>(),
            );
    ...
    pub(crate) fn eval_memory_load<CB: ChipBuilder<F>>(
        ...
        builder.assert_eq(
            ...
            (local.opcode_selector.is_lb + local.opcode_selector.is_lh)
        ...
        let signed_value = Word([
            CB::Expr::zero(),
            CB::Expr::one() * local.opcode_selector.is_lb,
            CB::Expr::one() * local.opcode_selector.is_lh,
            CB::Expr::zero(),
        ]);
        ...
        let mem_value_is_pos = (local.opcode_selector.is_lb + local.opcode_selector.is_lh)
            * (CB::Expr::one() - memory_columns.most_sig_byte_decomp[7])
            + local.opcode_selector.is_lbu
            + local.opcode_selector.is_lhu
            + local.opcode_selector.is_lw;
        builder.assert_eq(
    ...
    pub(crate) fn eval_memory_store<CB: ChipBuilder<F>>(
        ...
        let offset_is_zero = CB::Expr::one()
            - memory_columns.offset_is_one
            - memory_columns.offset_is_two
            - memory_columns.offset_is_three;
    ...
    /// instructions.
    pub(crate) fn eval_unsigned_mem_value<CB: ChipBuilder<F>>(
        &self,
        builder: &mut CB,
        memory_columns: &MemoryColumns<CB::Var>,
        local: &CpuCols<CB::Var>,
    ) {
        let mem_val = *memory_columns.memory_access.value();
    ...
    pub(crate) fn eval_most_sig_byte_bit_decomp<CB: ChipBuilder<F>>(
        &self,
        builder: &mut CB,
        memory_columns: &MemoryColumns<CB::Var>,
        local: &CpuCols<CB::Var>,
        ...
        let is_mem = self.is_memory_instruction::<CB>(&local.opcode_selector);
        let mut recomposed_byte = CB::Expr::zero();
        for i in 0..8 {
            builder
                .when(is_mem.clone())
            ...
            recomposed_byte +=
                memory_columns.most_sig_byte_decomp[i] * CB::Expr::from_canonical_u8(1 << i);
        }
        builder
            .when(local.opcode_selector.is_lb)
            .assert_eq(recomposed_byte.clone(), unsigned_mem_val[0]);
        builder
            .when(local.opcode_selector.is_lh)
            .assert_eq(recomposed_byte, unsigned_mem_val[1]);
    }

    /// Evaluates the offset value flags.
    pub(crate) fn eval_offset_value_flags<CB: ChipBuilder<F>>(
        &self,
        builder: &mut CB,
        memory_columns: &MemoryColumns<CB::Var>,
        local: &CpuCols<CB::Var>,
    ) {
        let is_mem_op = self.is_memory_instruction::<CB>(&local.opcode_selector);
        let offset_is_zero = CB::Expr::one()
            - memory_columns.offset_is_one
            - memory_columns.offset_is_two
            - memory_columns.offset_is_three;
        ...
        filtered_builder
            ...
            .assert_eq(memory_columns.addr_offset, CB::Expr::two());
        filtered_builder
            .when(memory_columns.offset_is_three)
            .assert_eq(memory_columns.addr_offset, CB::Expr::from_canonical_u8(3));
    }
}
