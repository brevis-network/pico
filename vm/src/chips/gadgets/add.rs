...
mod tests {
    ...
    fn test_add_gadget_simple_eval() {
        ...
        // create a new gadget
        let gadget = AddGadget {
            value: word,
            carry: [var; 3],
        };
        // create a constraint builder
        let mut builder = SymbolicConstraintFolder::new(0, size_of::<AddGadget<u8>>());

        // evaluate with this gadget
        AddGadget::<KoalaBear>::eval(&mut builder, word, word, gadget, Default::default());

        // check the constraints and public values
        assert_eq!(builder.constraints.len(), 14);
