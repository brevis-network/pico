...
mod tests {
    ...
    #[test]
    fn test_and_gadget_simple_eval() {
        let var = SymbolicVariable::new(Entry::Main { offset: 0 }, 0);
        let word = Word([var; 4]);

        // create a new gadget
        let gadget = AndOperation { value: word };
        // create a constraint builder
        let mut builder = SymbolicConstraintFolder::new(0, size_of::<AndOperation<u8>>());

        // evaluate with this gadget
        AndOperation::<KoalaBear>::eval(&mut builder, word, word, gadget, var);
