...
impl<const DEGREE: usize, F: Field> Poseidon2WideChip<DEGREE, F> {
    ...
    pub(crate) fn eval_state_transition<AB: ChipBuilder<F>>(
        &self,
        builder: &mut AB,
        ...
        // Absorb rows.
        {
            ...
            builder
                .when(control_flow.is_absorb)
                .when(local_opcode_workspace.absorb().do_perm::<F, AB>())
                .assert_all_eq(
        ...
        // Finalize rows.
        {
            ...
            builder
                ...
                .when(local_opcode_workspace.finalize().do_perm::<F, AB>())
                .assert_all_eq(
            ...
            builder
                .when(control_flow.is_finalize)
                .when_not(local_opcode_workspace.finalize().do_perm::<F, AB>())
                .assert_all_eq(
                    local_opcode_workspace.finalize().state,
