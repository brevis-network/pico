...
impl<const DEGREE: usize, F: Field> BaseAir<F> for MultiChip<DEGREE, F> {
    fn width(&self) -> usize {
...
impl<const DEGREE: usize, F: PrimeField32> ChipBehavior<F> for MultiChip<DEGREE, F> {
    ...
    fn generate_main(
        ...
        output: &mut RecursionRecord<F>,
    ) -> RowMajorMatrix<F> {
        let fri_fold_chip = FriFoldChip::<DEGREE, F> {
            fixed_log2_rows: None,
            pad: false,
            ..Default::default()
        };
        let poseidon2 = Poseidon2WideChip::<DEGREE, F> {
            fixed_log2_rows: None,
            pad: false,
            ..Default::default()
        };
        let fri_fold_trace = fri_fold_chip.generate_main(input, output);
        let mut poseidon2_trace = poseidon2.generate_main(input, output);

        let fri_fold_height = fri_fold_trace.height();
        let poseidon2_height = poseidon2_trace.height();

        let num_columns = <MultiChip<DEGREE, F> as BaseAir<F>>::width(self);
            ...
            .map(|(i, instruction_row)| {
                ...
                if process_fri_fold {
                    ...
                    multi_cols.fri_fold_receive_table =
                        FriFoldChip::<DEGREE, F>::do_receive_table(fri_fold_cols);
...
impl<AB, const DEGREE: usize, F: Field> Air<AB> for MultiChip<DEGREE, F>
where
    AB: ChipBuilder<F>,
    AB::Var: 'static,
{
    fn eval(&self, builder: &mut AB) {
        ...
        sub_builder.assert_eq(
            local_multi_cols.is_fri_fold
                * FriFoldChip::<DEGREE, F>::do_memory_access::<AB::Var>(&local_fri_fold_cols),
            local_multi_cols.fri_fold_memory_access,
        );
        sub_builder.assert_eq(
            local_multi_cols.is_fri_fold
                * FriFoldChip::<DEGREE, F>::do_receive_table::<AB::Var>(&local_fri_fold_cols),
            local_multi_cols.fri_fold_receive_table,
        );

        let fri_fold_chip = FriFoldChip::<DEGREE, F>::default();
        ...
        let poseidon2_columns = MultiChip::<DEGREE, F>::poseidon2(local_slice);
        sub_builder.assert_eq(
        ...
        let poseidon2_chip = Poseidon2WideChip::<DEGREE, F>::default();
        poseidon2_chip.eval_poseidon2(
            &mut sub_builder,
            poseidon2_columns.as_ref(),
            MultiChip::<DEGREE, F>::poseidon2(next_slice).as_ref(),
            local_multi_cols.poseidon2_receive_table,
...
impl<const DEGREE: usize, F: Field> MultiChip<DEGREE, F> {
    fn fri_fold_width() -> usize {
        <FriFoldChip<DEGREE, F> as BaseAir<F>>::width(&FriFoldChip::<DEGREE, F>::default())
    }
    ...
    fn poseidon2_width() -> usize {
        <Poseidon2WideChip<DEGREE, F> as BaseAir<F>>::width(
            &Poseidon2WideChip::<DEGREE, F>::default(),
        )
    }
        ...
        T: Copy + 'a,
    {
        ...
        Poseidon2WideChip::<DEGREE, F>::convert::<T>(
