...
pub trait Posedion2BabyBearHasherVariable<CC: CircuitConfig> {
...
impl FieldHasher<BabyBear> for BabyBearPoseidon2 {
    ...
    fn constant_compress(input: [Self::Digest; 2]) -> Self::Digest {
        ...
        (pico_poseidon2bb_init()).permute_mut(&mut pre); // TODO: change to riscv_perm()
        pre[..DIGEST_SIZE].try_into().unwrap()
    }
}

impl<CC: CircuitConfig<F = BabyBear>> Posedion2BabyBearHasherVariable<CC> for BabyBearPoseidon2 {
    fn poseidon2_permute(
        builder: &mut Builder<CC>,
...
impl<CC: CircuitConfig<F = BabyBear, Bit = Felt<BabyBear>>> FieldHasherVariable<CC>
    for BabyBearPoseidon2
{
    ...
    fn hash(
        ...
        <Self as Posedion2BabyBearHasherVariable<CC>>::poseidon2_hash(builder, input)
    }
...
impl<CC: CircuitConfig> Posedion2BabyBearHasherVariable<CC> for EmbedSC {
    fn poseidon2_permute(
        builder: &mut Builder<CC>,
        state: [Felt<<CC>::F>; PERMUTATION_WIDTH],
    ) -> [Felt<<CC>::F>; PERMUTATION_WIDTH] {
...
impl FieldHasher<BabyBear> for EmbedSC {
...
impl<CC: CircuitConfig<F = BabyBear, N = Bn254Fr, Bit = Var<Bn254Fr>>> FieldHasherVariable<CC>
    for EmbedSC
{
    ...
    fn hash(
        ...
        assert!(CC::F::bits() == p3_baby_bear::BabyBear::bits());
