...
pub enum RiscvChipType<F: Field> {
    ...
    MemoryReadWrite(MemoryReadWriteChip<F>),
    AddSub(AddSubChip<F>),
...
impl<F: PrimeField32> ChipBehavior<F> for RiscvChipType<F> {
    ...
    fn name(&self) -> String {
        match self {
            ...
            Self::MemoryReadWrite(chip) => chip.name(),
    ...
    fn generate_preprocessed(&self, program: &Program) -> Option<RowMajorMatrix<F>> {
        match self {
            ...
            Self::MemoryReadWrite(chip) => chip.generate_preprocessed(program),
    ...
    fn generate_main(&self, input: &Self::Record, output: &mut Self::Record) -> RowMajorMatrix<F> {
        match self {
            ...
            Self::MemoryReadWrite(chip) => chip.generate_main(input, output),
    ...
    fn preprocessed_width(&self) -> usize {
        match self {
            ...
            Self::MemoryReadWrite(chip) => chip.preprocessed_width(),
    ...
    fn extra_record(&self, input: &mut Self::Record, extra: &mut Self::Record) {
        match self {
            ...
            Self::MemoryReadWrite(chip) => chip.extra_record(input, extra),
    ...
    fn is_active(&self, record: &Self::Record) -> bool {
        match self {
            ...
            Self::MemoryReadWrite(chip) => chip.is_active(record),
...
impl<F: Field> BaseAir<F> for RiscvChipType<F> {
    fn width(&self) -> usize {
        match self {
            ...
            Self::MemoryReadWrite(chip) => chip.width(),
    ...
    fn preprocessed_trace(&self) -> Option<RowMajorMatrix<F>> {
        match self {
            ...
            Self::MemoryReadWrite(chip) => chip.preprocessed_trace(),
    ...
    CB: ChipBuilder<F>,
{
    fn eval(&self, b: &mut CB) {
        match self {
            ...
            Self::MemoryReadWrite(chip) => chip.eval(b),
