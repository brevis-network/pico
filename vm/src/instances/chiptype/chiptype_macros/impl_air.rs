#[macro_export]
macro_rules! impl_air {
    ($enum_name:ident, $F:ident, [ $( ($variant:ident, $chip_type:ident) ),+ ]) => {

        impl<$F: PrimeField32, CB: ChipBuilder<$F>, const HALF_EXTERNAL_ROUNDS: usize, const NUM_INTERNAL_ROUNDS: usize> Air<CB> for $enum_name<$F, HALF_EXTERNAL_ROUNDS, NUM_INTERNAL_ROUNDS>
        where Poseidon2PermuteChip<$F, HALF_EXTERNAL_ROUNDS, NUM_INTERNAL_ROUNDS>: BaseAir<$F> {
            fn eval(&self, b: &mut CB) {
                match self {
                    $( Self::$variant(chip) => chip.eval(b), )+
                }
            }
        }
    };
}
