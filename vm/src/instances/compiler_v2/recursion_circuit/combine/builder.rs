...
impl<CC, SC> RecursionCombineVerifierCircuit<CC, SC>
where
    ...
    CC: CircuitConfig<F = SC::Val, EF = SC::Challenge>,
{
    pub fn build_verifier(
        ...
        machine: &BaseMachine<SC, RecursionChipType<SC::Val, RISCV_COMPRESS_DEGREE>>,
        ...
        */
        ...
        for (i, (vk, chunk_proof)) in vks_and_proofs.into_iter().enumerate() {
             ...
             */
            {
                ...
                for is_non_zero in is_non_zero_flags {
                        ...
                        .zip(current_public_values.committed_value_digest)
                    {
                        ...
                        {
                            builder
                                .assert_felt_eq(is_non_zero * (byte_current - byte_public), zero);
                        }
                    }
                }
            ...
            for i in 0..ADDR_NUM_BITS {
                ...
                current_finalize_addr_bits[i] = current_public_values.last_finalize_addr_bits[i];
            }
