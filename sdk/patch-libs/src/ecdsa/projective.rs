//! Implementation of the Pico accelerated projective point. The projective point wraps the affine
//! point.
//!
...
use super::{AffinePoint, ECDSACurve, PicoAffinePointTrait};
...
/// The Pico accelerated projective point.
#[derive(Clone, Copy, Debug)]
pub struct ProjectivePoint<C: ECDSACurve> {
    /// The inner affine point.
    ///
    /// Pico uses affine arithmetic for all operations.
...
impl<C: ECDSACurve> ProjectivePoint<C> {
    ...
    fn to_zkvm_point(self) -> C::PicoAffinePoint {
        self.inner.inner
    }

    fn as_zkvm_point(&self) -> &C::PicoAffinePoint {
        &self.inner.inner
    }

    fn as_mut_zkvm_point(&mut self) -> &mut C::PicoAffinePoint {
    ...
    fn from_zkvm_point(p: C::PicoAffinePoint) -> Self {
        Self {
...
impl<C: ECDSACurve> LinearCombination for ProjectivePoint<C> {
    fn lincomb(x: &Self, k: &Self::Scalar, y: &Self, l: &Self::Scalar) -> Self {
        ...
        let pico_point =
            C::PicoAffinePoint::multi_scalar_multiplication(&a_bits_le, x, &b_bits_le, y);

        Self::from_zkvm_point(pico_point)
    }
}
...
impl<C: ECDSACurve, T: Borrow<C::Scalar>> Mul<T> for ProjectivePoint<C> {
    ...
    fn mul(mut self, rhs: T) -> Self::Output {
        let pico_point = self.as_mut_zkvm_point();
        pico_point.mul_assign(&be_bytes_to_le_words(rhs.borrow().to_repr()));
