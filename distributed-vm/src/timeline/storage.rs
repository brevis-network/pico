...
impl InMemStore {
    pub fn summarize_finished(&self) {
        ...
        let (mut single, multi): (Vec<_>, Vec<_>) = all
            .into_iter()
        ...
        struct RiscvRow {
            ...
            gen: Option<u128>,
            net_w2c: Option<u128>,
            riscv: Option<u128>,      // BeforeRiscv → AfterRiscv
            conv_setup: Option<u128>, // AfterRiscv → ConvertSetupDone
            conv: Option<u128>,       // ConvertSetupDone → ConvertDone
        }
        ...
        for tl in &all {
            match (tl.start_index, tl.end_index) {
                (idx, _) if idx == tl.end_index && idx != COORD_TL_ID => {
                    ...
                    let before_riscv = ts(tl, BeforeRiscv);
                    let after_riscv = ts(tl, AfterRiscv);
                    let conv_setup_done = ts(tl, ConvertSetupDone);
                    let conv_done = ts(tl, ConvertDone);
                    ...
                    riscv_rows.push(RiscvRow {
                        ...
                        riscv: ms(after_riscv, before_riscv),
                        conv_setup: ms(conv_setup_done, after_riscv),
                        conv: ms(conv_done, conv_setup_done),
                    });
                }
     ...
     {:>9} │ {:>13} │ {:>12}",
            ...
            writeln!(&mut buf, "{}", "─".repeat(140)).unwrap();
